...
import {
  ...
  FileText, Eye, ArrowDownToLine, Warehouse, Truck, StickyNote,
} from "lucide-react";
...
import { formatCurrency, canCreateMovements } from "@/lib/utils";
...
export default function EntradaStockPage() {
  ...
  const updateLine = (key: string, field: string, value: string) => {
    setLines((prev) => prev.map((l) => {
      ...
      if (field === "product_id") {
        ...
        return { ...l, product_id: value, product_name: prod?.name || "", sku: prod?.sku || "", unit_price: prod?.purchase_price ? String(prod.purchase_price) : l.unit_price, matched: true };
      }
  ...
  const handleOcr = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsed.supplier_name && !supplierId) {
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    if (fail > 0) sileo.warning({ title: `${ok} entradas creadas, ${fail} fallaron` });
    else sileo.success({ title: `${ok} entradas de stock registradas` });
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl font-bold tracking-tight">Entrada de Stock</h1>
              <Badge className="bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800 uppercase text-[10px] font-bold">
          ...
          {lines.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="size-16 rounded-2xl bg-muted/50 flex items-center justify-center mb-4">
                <ArrowDownToLine className="h-7 w-7 text-muted-foreground/50" />
              </div>
                    ...
                    return (
                      <tr key={line.key} className={`hover:bg-muted/20 group transition-colors ${needsMatch ? "bg-amber-50/50 dark:bg-amber-950/20" : ""}`}>
                        ...
                        <td className="py-3 px-2">
                          <input type="number" value={line.quantity} onChange={(e) => updateLine(line.key, "quantity", e.target.value)} placeholder="0" className="w-full text-right text-sm bg-background border border-border rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary font-mono" />
                        </td>
                        <td className="py-3 px-2">
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="pt-4">
          <textarea
            ...
            placeholder="Observaciones sobre esta entrada de stock (opcional)..."
            rows={3}
      ...
      <div className="flex items-center justify-between pb-8">
        ...
        <Button onClick={handleSave} disabled={saving || validCount === 0 || !warehouseId} className="gap-2 min-w-[200px]">
          {saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
          {saving ? "Registrando..." : `Registrar ${validCount} entrada${validCount !== 1 ? "s" : ""}`}
        </Button>
      </div>
    </div>
  );
